  ...
  class Parser
        ...
        @@file_types.each { |filetype|
          begin
            ...
            if filetype == PDF and @options[:force] == true
              ...
              info "No PDF header found, looking forward..."
              unless stream.skip_until(/%PDF-/).nil?
                stream.pos = stream.pos - 5

                warn "PDF header found at offset 0x#{stream.pos.to_s(16)} !"
                retry
              end
            end
    ...
    def read_xreftable(pdf, stream) #:nodoc:
      ...
      begin
        ...
        stream.pos -= 'trailer'.length unless stream.skip_until(/trailer/).nil?
      end
